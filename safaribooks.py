...
"""
    ...
    def book_info(self, info):
        ...
        for t in [
            ...
            self.info(
                "{0}{1}{2}: {3}".format(self.SH_YELLOW, t[0], self.SH_DEFAULT, t[1]),
                True,
            )
...
class SafariBooks:
    ...
    def __init__(self, args):
        ...
        if not self.cover:
            self.cover = (
                self.get_default_cover() if "cover" in self.book_info else False
            )
    ...
    def get_default_cover(self):
        ...
        if response == 0:
            self.display.error(
                "Error trying to retrieve the cover: %s" % self.book_info["cover"]
            )
            return False
            ...
            for chapter_css_url in self.chapter_stylesheets:
                ...
                page_css += (
                    ...
                    'rel="stylesheet" type="text/css" />\n'.format(
                        self.css.index(chapter_css_url)
                    )
                )
            ...
            if "images" in next_chapter and len(next_chapter["images"]):
                for img_url in next_chapter["images"]:
                    if api_v2_detected:
                        ...
                        self.images.append(
                            urljoin(next_chapter["asset_base_url"], img_url)
                        )
...
if __name__ == "__main__":
    ...
    arguments.add_argument(
        ...
        dest="use_spl",
        action="store_true",
        help="If set, uses Seattle Public Library EZ Proxy",
    )
